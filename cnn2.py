...
"""
# 第一层
# 卷积核(filter)的尺寸是5*5, 通道数为1，6，即feature map 数目为6
...
"""
W_conv1 = weight_variable([5, 5, 1, 6])  # 卷积是在每个5*5的patch中算出32个特征，分别是patch大小，输入通道数目，输出通道数目
...
# 第三层 是个全连接层,输入维数5*5*16, 输出维数为120
...
# h_fc1_flat = tf.reshape(h_pool2, [-1, 5 * 5 * 16])
# h_fc1 = tf.nn.elu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)

# 第五层，输入84维，输出10维，也就是具体的0~9分类
